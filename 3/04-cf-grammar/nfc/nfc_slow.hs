...
waysToAccept s g = runST run where
    ...
    run = do
        let n = length s
        a <- newArray ((0, 0, 'A'), (n, n, 'Z')) Nothing
        let finishes = do
                        rule <- states g
        ...
        sequence_ $! do
            ...
            t <- states g
            Finish c <- rules g t
            return $! writeArray a (i, i+1, t) $! Just $! waysToProduceOneLetter M.! (t, c)
        dp a 0 (start g) s
    
    turns :: [Char] -> [(Int, [Char], [Char])]
    turns xs = init $! tail $! zip3 [0..] (inits xs) (tails xs)

    dp :: STArray s (Int, Int, Char) (Maybe Int64) -> Int -> Char -> [Char] -> ST s Int64
    dp d pos rule s = do
        let n = length s
        ...
        case val of 
            ...
            Nothing -> do
                ...
                writeArray d (pos, pos + n, rule) $! Just ans
                return ans
...
main = do
    ...
    line <- hGetLine inp
    writeFile "ncf.out" $! show $! waysToAccept line g
