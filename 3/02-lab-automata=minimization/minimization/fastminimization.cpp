...
//        for (auto p : g[i])
//            gInv[p.second][p.first].push_back(i);
    vector<size_t> q;
    q.push_back(0);
    q.push_back(1);
    int p1 = 0, p2 = 1;
...
//        for (auto p : g[i])
//            newG[newID[eqlass[i]]][p.first] = newID[eqlass[p.second]];
